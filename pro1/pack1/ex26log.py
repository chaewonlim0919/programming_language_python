# 첨도, 왜도가 큰 (편차가 큰 데이터....) 데이터를 로그 변환하면 
# 분포를 개선, 범위 차이 축소 등으로 인해 모델을 안정적으로 수행 가능 -> 정규성을 높일 수 있다.
# y = log(x + 약간의수) # log는 0이나 음수는 갖을수 없다.

import math

class LogTrans:
    def __init__(self, offset:float=1.0): # offset 오류방지
        self.offset = offset
...
def main():
    ...
    print('역변한 자료 : ', reversed_data_round)
